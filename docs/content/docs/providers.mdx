...
aix uses React Context to share state between the message list, composer, and
animations. You need to wrap your chat screen with these providers.
...
Tracks the height of your composer input. This is used to calculate scroll
offsets and content insets.
...
Manages message list state including scroll position, content size, and blank
space calculations.
...
function Composer() {
  const { isMessageSendAnimating, setIsMessageSendAnimating } =
    useChatAnimation()
...
Tracks keyboard visibility state. This supplements
`react-native-keyboard-controller` with additional state management for
