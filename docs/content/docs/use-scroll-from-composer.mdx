---
title: useScrollMessageListFromComposerSizeUpdates
description: Auto-scroll when the composer grows
---

`useScrollMessageListFromComposerSizeUpdates` automatically scrolls the message list to the end when the composer height increases. This keeps the conversation visible as the user types multi-line messages.

## Usage

```tsx
import { useScrollMessageListFromComposerSizeUpdates } from 'aix'
...
The hook watches the `composerHeight` shared value. When it increases, it checks if the user is already scrolled near the bottom:
...
- [useComposerHeightContext](/docs/providers#composerheightcontextprovider) – Track composer height
